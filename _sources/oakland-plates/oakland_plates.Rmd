...
plates that they've seen — and where and when they saw each
one.
...
for i, row in jean_quan.iterrows():
    folium.Marker((row['Latitude'], row['Longditude']),
                  popup=row['Timestamp']).add_to(oak_map)
oak_map
```
...
```{python}
def map_plate(plate, zoom_start=12):
    sightings = lprs[lprs['Plate'] == plate]
    oak_map = make_oakland_map(zoom_start=zoom_start)
...
```{python}
map_plate('6UZA652', zoom_start=13)
```
