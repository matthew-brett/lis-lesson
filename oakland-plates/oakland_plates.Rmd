...
plates that they've seen — and where and when they saw each one.
...
lprs = pd.read_csv('all_lprs.csv', parse_dates=['Timestamp'])
lprs
```
...
Let's start by seeing what can be learned about someone, using this data —
