...
files, but two contained data that were already available in other files. The
...
```{python}
all_lprs = all_lprs.rename(columns={'red_vrm': 'Plate', 'red_timestamp': 'Timestamp'})
all_lprs.head()
```
...
all_lprs.to_csv('all_lprs.csv', index=None)
```
